...
export default class WorkCards {
    ...
    positionStyles = [
        ...
        'transform: translateX(-110%) scale(0.9); ', // Left
    ...
    moveBack() {
        if (this.currentItemIndex != this.cardCount && !this.itemsAreMoving && document.getElementById('work-item-0').classList.contains('work-item-container-transition')) {
    ...
    updatePositions(force = false) {
        if (!this.itemsAreMoving || force) {
            this.render.items.forEach((item) => {
                ...
                //update style class
                if (index + this.currentItemIndex != this.cardCount) {
    ...
    updateNavigation() {
        ...
        } else if (this.currentItemIndex == this.cardCount) {
